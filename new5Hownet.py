...
class How_Similarity:
    ...
    def __init__(self):
        ...
        self.full_set = set()  # 所有新建立的抽象概念集合。
        self.vocab = set()
    ...
    def loadFormula(self):
        '''        加载全部表达式        '''
        with open('ZW0305.txt', 'r', encoding='utf-8') as reader:
            try:
                lines = reader.readlines()
    ...
    def common_Father(self, w1, w2):
        ...
        order = sorted(common)  # 选取策略应该是深度最大的公共祖先
        return str(order[-1])  # 就是最近公共概念。

    def give_depth(self):
        '''给定一个概念，获得其深度。路径是唯一的。
           给定一个单词。因为有多条路径通往根节点，路径不唯一。深度取最小值？
           给每个单词/概念标注深度。执行策略：
           1，对于father == 自己的节点，深度为0。这些节点组成listA
        ...
        total = len(self.sememetable_)
        iter_list = []
        for key in self.sememetable_.keys():
            if key in self.sememetable_[key].father:
                self.sememetable_[key].depth = 0
                total -=1
                iter_list.append(key)  # 把当前节点加入下次迭代
        now_depth = 1

        while total > 0:
            # 第二次进入时。迭代范围还是全部。但father范围是in iterlist。
            next_iter =[]
            for key in self.sememetable_.keys():
                if self.sememetable_[key].depth >=0:
                     continue
                for fa in self.sememetable_[key].father:
                    if fa in iter_list:                    # 任何一个父亲在本次迭代名单中。则执行操作。
                        self.sememetable_[key].depth = now_depth
                        #print(self.sememetable_[key])
                        next_iter.append(key)        # 把当前节点加入
                        total -= 1
                        break
            now_depth +=1
            iter_list = next_iter
    ...
    def depth_of(self,concept):
        return self.sememetable_[self.sememeindex_zn_[concept].id].depth
...
if __name__ == '__main__':
    aa = How_Similarity()
    aa.loadSememeTable('whole0305.dat')
    aa.loadFormula()
    aa.give_fathers()
    # aa.give_depth()
    # 分配完父节点之后，就可以遍历义原表来给所有节点分配深度了。
    # for key in aa.sememetable_.keys():
    #     print(aa.sememetable_[key].id, aa.sememetable_[key].sememe_zh,
    #           aa.sememetable_[key].depth)

    #

    with open('father04.txt','a',encoding='utf-8') as f:
        for ele in aa.sememeindex_zn_.values():
            print(ele.id, ele.sememe_zh, ele.father, )
            line = ele.id +'|'+ ele.sememe_zh+'|'
            for a in  ele.father:
                line +=  a
                line += ','
            f.write(line+'\n')

    #print(aa.depth_of('楼房'),aa.depth_of('工地'))
    # fa = aa.common_Father('楼房','工地')
    # print('最近公共父节点是：',fa,aa.sememetable_[fa].sememe_zh)
