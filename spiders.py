...
class Proxy_Spider(object):
    ...
    def crawl_for_init(self):
        self.__get_All89()
        #self.__get_qq_room()
        #self.__get_Ai_Jia()  # 8篇文章总量1500多。每小时更新1篇文章。
        #self.__xiao_hexia()  # 更新频繁，但可用率低。启动采集
        #self.__xiao_shu()    # 每天更新2篇文章。只适合一次性采集
        #self.__get_code_busy()  # 更新很频繁，初次启动时可以更新一下。后面定期再采集

        return self.proxies_got

    def __rows_from(self, url, exp=None):  # 从网页表格中提取，小河虾、codeBusy采用了这种方式。
    ...
    def __get_code_busy(self):
        urls = ['https://proxy.coderbusy.com/classical/anonymous-type/highanonymous.aspx?page={}'.format(str(i)) for i
                in range(1, 10)]
        for url in urls:
    ...
    def __get_qq_room(self):
        ...
        for page in page_list[:3]:  # 只收集包含"代理ip"的前3篇文章
